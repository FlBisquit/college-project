...
def chat_main(request):
    if user := User.is_authorized(request):
        chats = Server.objects.filter(done=False)
...
def delete_chat(request, chat_id):
    if user := User.is_authorized(request):
        Server.objects.filter(id=chat_id).delete()
        return HttpResponseRedirect('/users')
    return HttpResponseRedirect('/')

def join_to_chat(request, chat_id):
    if user := User.is_authorized(request):
        chat = Server.objects.filter(id=chat_id).first()
        if chat:
